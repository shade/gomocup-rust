...
mod test {
    ...
    fn test_invalid_args_fails(#[case] input: Vec<&str>) {
        ...
        assert_matches!(cmd.execute(&mut GameContext::default(), args), Err(CommandError::InvalidArguments(_)))
    }

    #[test]
    fn test_calls_place() {
        let mut context = GameContext::default();
        let mut board = MockGameBoard::new();
        board.expect_place()
            .with(eq(1), eq(2))
            .times(1)
            .returning(|_, _| Ok(()));

        context.board = Some(Box::new(board));

        let cmd = TurnCommand::default();
        let args = vec!["1,2"].into_iter().map(|s| s.to_string()).collect();

        assert_matches!(cmd.execute(&mut context, args), Ok(CommandResult::Continue));
    }
